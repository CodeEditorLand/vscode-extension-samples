...
export function isTsxToolUserMetadata(
	obj: unknown,
): obj is TsxToolUserMetadata {
	...
	return (
		...
		Array.isArray(
			(obj as TsxToolUserMetadata).toolCallsMetadata.toolCallRounds,
		)
	);
}

export function registerToolUserChatParticipant(
		...
		const runWithTools = async (): Promise<void> => {
			...
			if (toolCalls.length) {
				...
				const toolResultMetadata =
					result.metadatas.getAll(ToolResultMetadata);

				if (toolResultMetadata?.length) {
					// Cache tool results for later, so they can be incorporated into later prompts without calling the tool again
					toolResultMetadata.forEach(
						(meta) =>
							(accumulatedToolResults[meta.toolCallId] =
								meta.result),
					);
				}
	...
	const toolUser = vscode.chat.createChatParticipant(
