...
fn main() -> Result<(), Box<dyn Error + Sync + Send>> {
	// Note that  we must have our logging only write out to stderr since the
	// communication with the client is done via stdin/stdout. If we write to
	// stdout, we will corrupt the communication.
